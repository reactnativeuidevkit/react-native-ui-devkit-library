...
const FunctionsMarginBottom = (props) => {
    useContext(GlobalContext);
    const colors = useColors();

    return (
        <ScrollView>
            <Header type={'marginBottom'} />

            <Item data={{
                component:
                    <Text style={[DescriptionFontSize(), { margin: 15, color: colors.text }]}>
                        {`Applies marginBottom() according to platform and version to any component that accepts the marginBottom property in its style.`}
                    </Text>,
                padding: false
            }}
...
export default FunctionsMarginBottom;
