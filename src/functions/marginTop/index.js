...
const FunctionsMarginTop = (props) => {
    useContext(GlobalContext);
    const colors = useColors();

    return (
        <ScrollView>
            <Header type={'marginTop'} />

            <Item data={{
                component:
                    <Text style={[DescriptionFontSize(), { margin: 15, color: colors.text }]}>
                        {`Applies marginTop() according to platform and version to any component that accepts the marginTop property in its style.`}
                    </Text>,
                padding: false
            }}
...
export default FunctionsMarginTop;
